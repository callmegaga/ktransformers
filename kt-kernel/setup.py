...
GPU backends:
  ...
  CPUINFER_USE_SYCL=0/1           -DKTRANSFORMERS_ADD_SYCL (experimental SYCL FP8 MoE)
...
class CMakeBuild(build_ext):
    ...
    def _build_single_variant_impl(self, ext: CMakeExtension, extdir: Path, build_temp: Path, cfg: str):
        ...
        if _env_get_bool("CPUINFER_USE_SYCL", False):
            cmake_args.append("-DKTRANSFORMERS_ADD_SYCL=ON")
            print("-- Enabling SYCL backend (-DKTRANSFORMERS_ADD_SYCL=ON)")
