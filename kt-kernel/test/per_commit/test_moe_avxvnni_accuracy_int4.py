#!/usr/bin/env python
# coding=utf-8
"""AVX-VNNI INT4 MoE accuracy tests for KT-Kernel."""
...
@pytest.mark.cpu
def test_moe_avxvnni_int4_accuracy():
    if not HAS_DEPS:
        pytest.skip(f"Dependencies not available: {import_error}")
    if not amx_utils._HAS_AVXVNNI256_INT4_SUPPORT or not amx_utils._HOST_HAS_AVX_VNNI:
        pytest.skip("AVX-VNNI INT4 backend not available on this host")
    ...
    with torch.inference_mode():
        ...
        moe = kt_kernel_ext.moe.AVXVNNI256Int4_MOE(config)
        ...
        for i in range(validation_iter):
            ...
            assert diff < 0.35, f"AVX-VNNI INT4 accuracy test failed: diff={diff:.6f} >= 0.35"
