#!/usr/bin/env python
# coding=utf-8
"""AWQ INT8 MoE accuracy tests for KT-Kernel x86 backends."""
...
def awq_sym_int8_quantize(weight_bf16: "torch.Tensor"):
    """Quantize [N, K] BF16 weight to AWQ symmetric INT8 layout."""
    n, k = weight_bf16.shape
    assert k % 4 == 0
    assert k % group_size == 0

    weight_fp32 = weight_bf16.float()
    packed = torch.zeros((n, k // 4), dtype=torch.int32)
...
def awq_sym_int8_dequantize(packed: "torch.Tensor", scales: "torch.Tensor", out_features: int, in_features: int):
    ...
    for ni in range(out_features):
        for g in range(in_features // group_size):
            ...
            k_end = k_start + group_size
            for kk in range(k_start, k_end, 4):
                word = int(packed[ni, kk // 4].item())
                for byte_idx in range(4):
                    raw = (word >> (byte_idx * 8)) & 0xFF
                    if raw >= 128:
                        raw -= 256
                    result[ni, kk + byte_idx] = raw * scale
    return result
...
@pytest.mark.cpu
def test_moe_awq_int8_accuracy():
    if not HAS_DEPS:
        pytest.skip(f"Dependencies not available: {import_error}")
    if not amx_utils._HAS_AVXVNNI256_AWQ_INT8_SUPPORT or not amx_utils._HOST_HAS_AVX_VNNI:
        pytest.skip("AVX-VNNI AWQ INT8 backend not available on this host")
    ...
    with torch.inference_mode():
        ...
        for e in range(expert_num):
            qw, sc = awq_sym_int8_quantize(gate_bf16[e])
            gate_qw_list.append(qw)
            gate_scale_list.append(sc)

            qw, sc = awq_sym_int8_quantize(up_bf16[e])
            up_qw_list.append(qw)
            up_scale_list.append(sc)

            qw, sc = awq_sym_int8_quantize(down_bf16[e])
        ...
        gate_deq = torch.stack(
            [
                awq_sym_int8_dequantize(gate_qw_list[e], gate_scale_list[e], intermediate_size, hidden_size)
                for e in range(expert_num)
            ]
        )
        up_deq = torch.stack(
            [
                awq_sym_int8_dequantize(up_qw_list[e], up_scale_list[e], intermediate_size, hidden_size)
                for e in range(expert_num)
            ]
        )
        down_deq = torch.stack(
            [
                awq_sym_int8_dequantize(down_qw_list[e], down_scale_list[e], hidden_size, intermediate_size)
                for e in range(expert_num)
            ]
        )
        ...
        moe = kt_kernel_ext.moe.AVXVNNI256AWQInt8_MOE(config)
        ...
        for i in range(validation_iter):
            ...
            assert diff < 0.08, f"AWQ INT8 accuracy test failed: diff={diff:.6f} >= 0.08"
