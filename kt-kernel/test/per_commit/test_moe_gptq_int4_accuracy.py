...
def available_backends():
    ...
    if os.environ.get("KT_TEST_SYCL_GPTQ_INT4") == "1" and hasattr(kt_kernel_ext.moe, "SYCLGPTQInt4_MOE"):
        ...
        if hasattr(kt_kernel_ext.moe, "CPUiGPUGPTQInt4_MOE"):
            ...
            backends.append(("CPUiGPUGPTQInt4_MOE_dynamic", kt_kernel_ext.moe.CPUiGPUGPTQInt4_MOE, 0.20))
    return backends
...
def run_backend_accuracy_test(backend_name, backend_cls, threshold, qlen_cases):
    ...
    with torch.inference_mode():
        ...
        if backend_name.startswith("CPUiGPUGPTQInt4_MOE"):
        ...
        elif backend_name == "CPUiGPUGPTQInt4_MOE_dynamic":
            config.cpu_igpu_dynamic = True
...
def test_cpu_igpu_scheduler_defaults_to_dynamic(monkeypatch):
    ...
    assert config.cpu_igpu_decode_load_low == 0.45
    assert config.cpu_igpu_decode_load_high == 0.55
    assert config.cpu_igpu_prefill_load_low == 0.65
    assert config.cpu_igpu_prefill_load_high == 0.75
    assert config.cpu_igpu_load_ewma_alpha == 0.25
    assert config.cpu_igpu_load_sample_ms == 50
    assert config.cpu_igpu_decode_min_dwell == 4
    assert config.cpu_igpu_prefill_min_dwell == 2
...
def test_cpu_igpu_scheduler_rejects_invalid_policy(monkeypatch):
    ...
    with pytest.raises(ValueError, match="must be one of: dynamic, fixed, phase-fixed"):
...
@pytest.mark.parametrize(
    ("name", "value", "message"),
    (
        ("KT_CPU_IGPU_RATIO", "1.5", "fixed ratios"),
        ("KT_CPU_IGPU_DECODE_LOAD_LOW", "0.9", "load thresholds"),
        ("KT_CPU_IGPU_LOAD_SAMPLE_MS", "5", "at least 10"),
        ("KT_CPU_IGPU_PREFILL_MIN_DWELL", "0", "minimum dwell"),
    ),
)
def test_cpu_igpu_scheduler_rejects_invalid_values(monkeypatch, name, value, message):
    amx_utils = load_amx_utils()
