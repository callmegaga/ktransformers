...
struct GemmKernelAVXVNNI256PackedGPTQInt4 {
  ...
  struct BufferB {
    ...
    static size_t required_size(size_t n, size_t k, int group_size) {
      ...
      return n * k_packed * sizeof(uint32_t) + n * num_groups * sizeof(float) +
             n * num_groups * sizeof(int16_t);
    }
...
KT_PACKED_AVXVNNI256_TARGET
static inline void gemm_gptq_sym_int4_packed_avxvnni256(
    int m, int n, int k, GemmKernelAVXVNNI256PackedGPTQInt4::BufferA& activation,
    GemmKernelAVXVNNI256PackedGPTQInt4::BufferB& weight,
    GemmKernelAVXVNNI256PackedGPTQInt4::BufferC& output, int ith, int nth) {
  (void)k;
  auto [n_begin, n_end] = avx2::split_range(n, ith, nth);
  ...
  for (int row = 0; row < m; ++row) {
    ...
    for (int group = 0; group < num_groups; ++group) {
      ...
      for (int column = n_begin; column < n_end; ++column) {
        ...
        const int dot = avxvnni::hsum_epi32_avx2(accumulator) - 128 * static_cast<int>(weight.weight_sums[output_group]);
...
class AVXVNNI256_PACKED_GPTQ_INT4_MOE_TP
    : public AVX2_MOE_BASE<T, AVXVNNI256_PACKED_GPTQ_INT4_MOE_TP<T>> {
 ...
 public:
  ...
  AVXVNNI256_PACKED_GPTQ_INT4_MOE_TP(GeneralMOEConfig config, int tp_part_idx_ = 0)
      : Base(config, tp_part_idx_) {}
  ...
  void do_gate_up_gemm(bool do_up, int expert, int ith, int nth, int qlen) {
    ...
    avxvnni_packed::gemm_gptq_sym_int4_packed_avxvnni256(
        m_local_num_[expert], config_.intermediate_size, config_.hidden_size, *gate_up_ba_[expert], *weight, *output,
        ith, nth);
  }
  ...
  void load_weights() {
    ...
    const size_t gate_up_qweight_elements =
        static_cast<size_t>(config_.hidden_size / 8) * config_.intermediate_size;
    ...
    pool->do_work_stealing_job(
        ...
        [this, nth, physical_to_logical, per_expert, gate_up_qweight_elements, gate_up_scale_elements](int task) {
          ...
          const float* up_scale =
              per_expert ? static_cast<const float*>(config_.up_scales[0][logical])
    ...
    const size_t down_qweight_elements =
        static_cast<size_t>(config_.intermediate_size / 8) * config_.hidden_size;
    ...
    pool->do_work_stealing_job(
        ...
        [this, nth, physical_to_logical, per_expert, down_qweight_elements, down_scale_elements](int task) {
          ...
          const float* down_scale =
              per_expert ? static_cast<const float*>(config_.down_scales[0][logical])
