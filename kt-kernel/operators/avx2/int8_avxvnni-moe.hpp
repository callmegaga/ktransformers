/**
 * @Description  : AVX-VNNI-256 INT8 MoE operator
 * @Author       : Codex
 * @Date         : 2026-04-05
 * @Version      : 1.0.0
 * @Copyright (c) 2024 by KVCache.AI, All Rights Reserved.
 *
 * This backend mirrors AMXINT8 quantization semantics:
 * - Activations: per-row symmetric int8 quantization
 * - Weights: per-output-channel symmetric int8 quantization
 *
 * To use AVX-VNNI-256 (`vpdpbusd`) efficiently, activations are stored as
 * biased uint8 (`q ^ 0x80`) and weights are stored as signed int8. A per-row
 * weight sum is precomputed so signed-signed dot products can be reconstructed
 * exactly from unsigned-signed dpbusd accumulators.
 */
#ifndef CPUINFER_OPERATOR_AVX2_INT8_AVXVNNI_MOE_H
#define CPUINFER_OPERATOR_AVX2_INT8_AVXVNNI_MOE_H
...
struct GemmKernelAVXVNNI256Int8 {
  ...
  static std::string name() { return "AVXVNNI_INT8"; }
  ...
  struct BufferB {
    ...
    BufferB(size_t n_, size_t k_, void* ptr) : n((int)n_), k((int)k_) {
      if ((k % K_STEP) != 0) {
        throw std::runtime_error("AVX-VNNI INT8 requires k to be a multiple of 32");
      }
      set_data(ptr);
    }
    ...
    void from_quant_mat(const int8_t* src_weight, const float* src_scale, int ith, int nth) {
      auto [n_start, n_end] = avx2::split_range(n, ith, nth);
      for (int ni = n_start; ni < n_end; ++ni) {
        std::memcpy(b + (size_t)ni * k, src_weight + (size_t)ni * k, (size_t)k * sizeof(int8_t));
      }
...
static inline void gemm_int8_avxvnni256(int m, int n, int k, GemmKernelAVXVNNI256Int8::BufferA& a,
                                        GemmKernelAVXVNNI256Int8::BufferB& b, GemmKernelAVXVNNI256Int8::BufferC& c,
                                        int ith, int nth) {
  auto [n_start, n_end] = avx2::split_range(n, ith, nth);
...
template <class T = avxvnni_int8::GemmKernelAVXVNNI256Int8>
class AVXVNNI256_INT8_MOE_TP : public AVX2_MOE_BASE<T, AVXVNNI256_INT8_MOE_TP<T>> {
  using Base = AVX2_MOE_BASE<T, AVXVNNI256_INT8_MOE_TP<T>>;
  using Base::config_;
  using Base::down_ba_;
  using Base::down_bb_;
  using Base::down_bc_;
  ...
  inline void write_weights(std::filesystem::path prefix, std::string mat_class, typename T::BufferB* buffer,
    ...
    of.write((char*)buffer->b, quant_size);
    of.close();
  ...
  inline void read_weights(std::filesystem::path prefix, std::string mat_class, typename T::BufferB* buffer,
    ...
    f.read((char*)buffer->b, quant_size);
    f.close();
    ...
    buffer->recompute_weight_sums(0, buffer->n);
  }
 ...
 public:
  ...
  AVXVNNI256_INT8_MOE_TP() = default;
  AVXVNNI256_INT8_MOE_TP(GeneralMOEConfig config, int tp_part_idx_ = 0) : Base(config, tp_part_idx_) {}

  void derived_init() {
#if defined(__GNUC__) || defined(__clang__)
    if (!__builtin_cpu_supports("avxvnni")) {
      throw std::runtime_error("AVX-VNNI-256 INT8 backend requires CPU support for avx_vnni");
    }
#endif
    printf("Created AVXVNNI256_INT8_MOE_TP %d at numa %d\n", tp_part_idx, numa_node_of_cpu(sched_getcpu()));
  }
  ...
  void do_gate_up_gemm(bool do_up, int expert_idx, int ith, int nth, int qlen) {
    ...
    avxvnni_int8::gemm_int8_avxvnni256(m, config_.intermediate_size, config_.hidden_size, *ba, *bb, *bc, ith, nth);
  }

  void do_down_gemm(int expert_idx, int ith, int nth, int qlen) {
    (void)qlen;
    int m = m_local_num_[expert_idx];
    avxvnni_int8::gemm_int8_avxvnni256(m, config_.hidden_size, config_.intermediate_size, *down_ba_[expert_idx],
  ...
  void load_weights() {
    ...
    if (!config_.gate_projs.empty()) {
      pool->do_work_stealing_job(
          ...
          [this, physical_to_logical_map](int expert_id) {
            ...
            gate_bb_[expert_id]->from_quant_mat((const int8_t*)config_.gate_projs[tp_part_idx][logical_expert_id],
                                                (const float*)config_.gate_scales[tp_part_idx][logical_expert_id], 0, 1);
            up_bb_[expert_id]->from_quant_mat((const int8_t*)config_.up_projs[tp_part_idx][logical_expert_id],
                                              (const float*)config_.up_scales[tp_part_idx][logical_expert_id], 0, 1);
            down_bb_[expert_id]->from_quant_mat((const int8_t*)config_.down_projs[tp_part_idx][logical_expert_id],
    ...
    if (config_.load) {
      for (int expert_id = 0; expert_id < config_.expert_num; ++expert_id) {
        uint64_t logical_expert_id = expert_map(physical_to_logical_map, expert_id);
        read_weights(prefix, "_gate_", gate_bb_[expert_id].get(), (int)logical_expert_id,
                     (size_t)config_.intermediate_size * config_.hidden_size * sizeof(int8_t),
                     (size_t)config_.intermediate_size * sizeof(float));
        read_weights(prefix, "_up_", up_bb_[expert_id].get(), (int)logical_expert_id,
                     (size_t)config_.intermediate_size * config_.hidden_size * sizeof(int8_t),
                     (size_t)config_.intermediate_size * sizeof(float));
        read_weights(prefix, "_down_", down_bb_[expert_id].get(), (int)logical_expert_id,
                     (size_t)config_.hidden_size * config_.intermediate_size * sizeof(int8_t),
    ...
    if (config_.save) {
      ...
      pool->do_work_stealing_job(
          ...
          [this, physical_to_logical_map, prefix](int task_id) {
            ...
            uint64_t logical_expert_id = expert_map(physical_to_logical_map, expert_id);
            if ((task_id % 3) == 0) {
              write_weights(prefix, "_up_", up_bb_[logical_expert_id].get(), (int)logical_expert_id,
                            (size_t)config_.intermediate_size * config_.hidden_size * sizeof(int8_t),
                            (size_t)config_.intermediate_size * sizeof(float));
            } else if ((task_id % 3) == 1) {
              write_weights(prefix, "_gate_", gate_bb_[logical_expert_id].get(), (int)logical_expert_id,
                            (size_t)config_.intermediate_size * config_.hidden_size * sizeof(int8_t),
                            (size_t)config_.intermediate_size * sizeof(float));
            } else {
              write_weights(prefix, "_down_", down_bb_[logical_expert_id].get(), (int)logical_expert_id,
                            (size_t)config_.hidden_size * config_.intermediate_size * sizeof(int8_t),
                            (size_t)config_.hidden_size * sizeof(float));
            }
          },
  ...
  void write_weights_to_buffer(int gpu_tp_count, [[maybe_unused]] int cpu_tp_count, int expert_id,
    ...
    (void)w2_weight_ptrs;
    (void)w2_scale_ptrs;
    throw std::runtime_error("AVX-VNNI INT8 write_weights_to_buffer not yet implemented");
  }
};

template <typename K>
class TP_MOE<AVXVNNI256_INT8_MOE_TP<K>> : public TP_MOE<AVX2_MOE_BASE<K, AVXVNNI256_INT8_MOE_TP<K>>> {
 public:
  using Base = TP_MOE<AVX2_MOE_BASE<K, AVXVNNI256_INT8_MOE_TP<K>>>;
  using Base::Base;
...
#endif  // CPUINFER_OPERATOR_AVX2_INT8_AVXVNNI_MOE_H
