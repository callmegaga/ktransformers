/**
 * @Description  : AVX-VNNI-256 AWQ-Int8 MoE operator (group-wise symmetric quantization)
 * @Author       : Codex
 * @Date         : 2026-04-09
 * @Version      : 1.0.0
 *
 * Expected packed layout from real AWQ-8bit checkpoints:
 *   weight_packed [N, K/4] int32
 *   weight_scale  [N, K/group_size] bf16/fp32
 *   weight_shape  [N, K]
 **/
#ifndef CPUINFER_OPERATOR_AVX2_AWQ_INT8_AVXVNNI_MOE_H
#define CPUINFER_OPERATOR_AVX2_AWQ_INT8_AVXVNNI_MOE_H
...
#define KT_AVXVNNI256_AWQ_INT8_TARGET __attribute__((target("avx2,avxvnni,fma")))
#else
#define KT_AVXVNNI256_AWQ_INT8_TARGET
#endif

namespace avxvnni_awq_int8 {
...
struct GemmKernelAVXVNNI256AWQInt8 {
  ...
  struct BufferB {
    int8_t* qweight_s8 = nullptr;   // [N, K], signed int8
    ...
    BufferB(size_t n_, size_t k_, int gs, void* ptr) : n((int)n_), k((int)k_), group_size(gs) {
      if (group_size <= 0 || (group_size % 32) != 0) {
        throw std::runtime_error("AVX-VNNI AWQ INT8 requires group_size to be a positive multiple of 32");
      }
      if ((k % 4) != 0 || (k % group_size) != 0) {
        throw std::runtime_error("AVX-VNNI AWQ INT8 requires k to be divisible by both 4 and group_size");
      }
      k_packed = k / 4;
      ...
      for (int ni = n_start; ni < n_end; ++ni) {
        ...
        for (int kp = 0; kp < k_packed; ++kp) {
          ...
          for (int byte_idx = 0; byte_idx < 4; ++byte_idx) {
            ...
            dst_row[kp * 4 + byte_idx] = (int8_t)raw;
          }
        }
...
KT_AVXVNNI256_AWQ_INT8_TARGET
static inline void gemm_awq_int8_avxvnni256(int m, int n, int k, GemmKernelAVXVNNI256AWQInt8::BufferA& a,
                                            GemmKernelAVXVNNI256AWQInt8::BufferB& b,
                                            GemmKernelAVXVNNI256AWQInt8::BufferC& c, int ith, int nth) {
  (void)k;
  auto [n_start, n_end] = avx2::split_range(n, ith, nth);
...
template <class T = avxvnni_awq_int8::GemmKernelAVXVNNI256AWQInt8>
class AVXVNNI256_AWQ_INT8_MOE_TP : public AVX2_MOE_BASE<T, AVXVNNI256_AWQ_INT8_MOE_TP<T>> {
  using Base = AVX2_MOE_BASE<T, AVXVNNI256_AWQ_INT8_MOE_TP<T>>;
  using Base::config_;
  using Base::down_ba_;
  using Base::down_bb_;
  using Base::down_bc_;
 ...
 public:
  ...
  AVXVNNI256_AWQ_INT8_MOE_TP() = default;
  AVXVNNI256_AWQ_INT8_MOE_TP(GeneralMOEConfig config, int tp_part_idx_ = 0) : Base(config, tp_part_idx_) {}

  void derived_init() {
#if defined(__GNUC__) || defined(__clang__)
    if (!__builtin_cpu_supports("avxvnni")) {
      throw std::runtime_error("AVX-VNNI-256 AWQ_INT8 backend requires CPU support for avx_vnni");
    }
#endif
    auto& qc = config_.quant_config;
    if (qc.group_size == 0 || (qc.group_size % 32) != 0) {
      throw std::runtime_error("AVX-VNNI-256 AWQ_INT8 requires group_size to be a positive multiple of 32");
    }
    printf("Created AVXVNNI256_AWQ_INT8_MOE_TP %d at numa %d (group_size=%d)\n", tp_part_idx,
  ...
  void do_gate_up_gemm(bool do_up, int expert_idx, int ith, int nth, int qlen) {
    ...
    avxvnni_awq_int8::gemm_awq_int8_avxvnni256(m, config_.intermediate_size, config_.hidden_size, *ba, *bb, *bc, ith, nth);
  }

  void do_down_gemm(int expert_idx, int ith, int nth, int qlen) {
    (void)qlen;
    int m = m_local_num_[expert_idx];
    avxvnni_awq_int8::gemm_awq_int8_avxvnni256(m, config_.hidden_size, config_.intermediate_size, *down_ba_[expert_idx],
  ...
  void load_weights() {
    ...
    if (config_.gate_scale == nullptr && config_.gate_scales.empty()) {
      throw std::runtime_error("AWQ INT8 MOE requires scale pointers.");
    }

    const int gate_up_k = config_.hidden_size;
    const int gate_up_n = config_.intermediate_size;
    const size_t gate_up_qw_elems = (size_t)gate_up_n * (gate_up_k / 4);
    ...
    const size_t down_qw_elems = (size_t)down_n * (down_k / 4);
  ...
  void write_weights_to_buffer(int gpu_tp_count, [[maybe_unused]] int cpu_tp_count, int expert_id,
    ...
    (void)w2_weight_ptrs;
    throw std::runtime_error("AVX-VNNI-256 AWQ INT8 write_weights_to_buffer not yet implemented");
  }
};

template <typename K>
class TP_MOE<AVXVNNI256_AWQ_INT8_MOE_TP<K>> : public TP_MOE<AVX2_MOE_BASE<K, AVXVNNI256_AWQ_INT8_MOE_TP<K>>> {
 public:
  using Base = TP_MOE<AVX2_MOE_BASE<K, AVXVNNI256_AWQ_INT8_MOE_TP<K>>>;
  using Base::Base;
  ...
  void load_weights() override {
    ...
    if (group_size == 0) {
      throw std::runtime_error("AWQ INT8 requires group_size > 0");
    }
  ...
  void write_weight_scale_to_buffer(int gpu_tp_count, int expert_id, const std::vector<uintptr_t>& w13_weight_ptrs,
    ...
    (void)w2_weight_ptrs;
    (void)w2_scale_ptrs;
    throw std::runtime_error("AVX-VNNI-256 AWQ INT8 write_weight_scale_to_buffer not yet implemented");
  }
};

#undef KT_AVXVNNI256_AWQ_INT8_TARGET

#endif  // CPUINFER_OPERATOR_AVX2_AWQ_INT8_AVXVNNI_MOE_H
