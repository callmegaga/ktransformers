...
def _configure_cpu_igpu_scheduler(config) -> None:
    ...
    if policy not in {"dynamic", "fixed", "phase-fixed"}:
        raise ValueError("KT_CPU_IGPU_POLICY must be 'dynamic', 'fixed', or 'phase-fixed'")

    float_options = {
        "cpu_igpu_decode_load_low": ("KT_CPU_IGPU_DECODE_LOAD_LOW", 0.10),
        "cpu_igpu_decode_load_high": ("KT_CPU_IGPU_DECODE_LOAD_HIGH", 0.20),
        "cpu_igpu_prefill_load_low": ("KT_CPU_IGPU_PREFILL_LOAD_LOW", 0.99),
        "cpu_igpu_prefill_load_high": ("KT_CPU_IGPU_PREFILL_LOAD_HIGH", 1.0),
        "cpu_igpu_load_ewma_alpha": ("KT_CPU_IGPU_LOAD_EWMA_ALPHA", 0.25),
        "cpu_igpu_cost_ewma_alpha": ("KT_CPU_IGPU_COST_EWMA_ALPHA", 0.20),
        "cpu_igpu_decode_switch_margin": ("KT_CPU_IGPU_DECODE_SWITCH_MARGIN", 0.10),
        "cpu_igpu_decode_cost_load_match_delta": ("KT_CPU_IGPU_DECODE_COST_LOAD_MATCH_DELTA", 0.10),
        "cpu_igpu_decode_load_reprobe_delta": ("KT_CPU_IGPU_DECODE_LOAD_REPROBE_DELTA", 0.25),
    }
    int_options = {
        ...
        "cpu_igpu_decode_calibration_samples": ("KT_CPU_IGPU_DECODE_CALIBRATION_SAMPLES", 32),
        "cpu_igpu_decode_load_reprobe_grace": ("KT_CPU_IGPU_DECODE_LOAD_REPROBE_GRACE", 64),
        "cpu_igpu_decode_reprobe_samples": ("KT_CPU_IGPU_DECODE_REPROBE_SAMPLES", 32),
        "cpu_igpu_decode_reprobe_interval": ("KT_CPU_IGPU_DECODE_REPROBE_INTERVAL", 4096),
    }
    try:
